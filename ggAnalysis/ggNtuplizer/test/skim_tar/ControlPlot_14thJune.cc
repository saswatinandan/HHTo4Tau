...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
    ...
    output.erase(root);
    output += "_controlPlot_25June.root";
    //output = ".root";
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
	    ...
	    if(tau1antiisotau2antiiso) {
	      fake1 = fakeweight(tauPt->at(vec_tau[itau]));
	      fake2 = fakeweight(tauPt->at(vec_tau[jtau]));
		...
		for(int muchrg=0; muchrg<2; muchrg++) {
		  ...
		  for(int tauchrg=0; tauchrg<2; tauchrg++) {
		    if(!taucharge[tauchrg]) continue;
		    std::string title = muCharge_[muchrg]+"_"+tauCharge_[tauchrg]+"_region";
		    plotFill("InvariantMass_of_tau_pair_with_"+title+tauiso,T.M(),6,50,110,weight*fake);
		    plotFill("pt_distribution_of_Leading_#tau_with_"+title+tauiso,tauPt->at(vec_tau[itau]),75,0,150,weight*fake);
		    plotFill("pt_distribution_of_SubLeading_#tau_with_"+title+tauiso,tauPt->at(vec_tau[jtau]),75,0,150,weight*fake);
		...
		for(int totchrg=0; totchrg<2; totchrg++) {
		  ...
		  plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso,HH.M(),7,250,900,weight*leading_weight*subleading_weight*fake);
		}
		if(taumucharge_[0]) {
		  bool muCharge_;
		  muCharge_ = chrg[imu]+chrg[jmu]==0;
		  mucharge_[0] = muCharge_ ? true : false;
		  mucharge_[1] = !muCharge_;
		  for(int muchrg=0; muchrg<2; muchrg++) {
		    if(muchrg !=1) continue;
		    if(!mucharge_[muchrg]) continue;
		    plotFill("InvariantMass_of_4_particle_with_"+taumucharge[muchrg]+tauiso,HH.M(),7,250,900,weight*leading_weight*subleading_weight*fake);
		    plotFill("InvariantMass_of_#mu_pair_with_"+taumucharge[muchrg]+tauiso,M.M(),30,60,120,weight*leading_weight*subleading_weight);
		    plotFill("InvariantMass_of_#tau_pair_with_"+taumucharge[muchrg]+tauiso,T.M(),45,60,150,weight*fake);
		  }
		  TLorentzVector H;
		  for(int i=0; i<2; i++) {
		    TLorentzVector tau = (i==0) ? t1 : t2;
		    int taucharge_ = (i==0) ? tauCharge->at(vec_tau[itau]) : tauCharge->at(vec_tau[jtau]);
		    for(int j=0; j<2; j++) {
		      TLorentzVector mu = (j==0) ? m1 : m2;
		      int mucharge_ = (j==0) ? chrg[imu] : chrg[jmu];
		      H = tau+mu;
		      std::string title = (taucharge_ + mucharge_ ==0) ? "opposite_sign" : "same_sign";
		      muonweight = (j==0) ? leading_weight : subleading_weight;
		      plotFill("InvariantMass_of_taumu_with_"+title+tauiso,H.M(),16,40,200,weight*fake*muonweight);
		      }
		  }
		}
	      }
...
double fakeweight(double pt)  {
  ...
  return .0194+.232*TMath::Exp(-.0653*pt);
}
