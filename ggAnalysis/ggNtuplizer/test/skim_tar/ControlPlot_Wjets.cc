	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
	    ...
	    for(int imu=0; imu<vec_muele.size(); imu++) {
	      ...
	      plotFill("InvariantMass_of_3_particle"+tauiso,HH.M(),30,50,200,fake);
	      plotFill("InvariantMass_of_3_particle_with_"+tauiso+"_nofake_weight",HH.M(),30,50,200);
	      TLorentzVector H;
	      for(int i=0; i<2; i++) {
		TLorentzVector tau = (i==0) ? t1 : t2;
		H = tau+m1;
		plotFill("InvariantMass_of_taumu_with_"+tauiso,H.M(),50,50,150,fake);
		plotFill("InvariantMass_of_taumu_with_"+tauiso+"_nofake_weight",H.M(),50,50,150);
	      }
	    }
	  }
	}
...
double fakeweight(double pt)  {
  ...
  return .002+.153*TMath::Exp(-.085*pt);///QCD
