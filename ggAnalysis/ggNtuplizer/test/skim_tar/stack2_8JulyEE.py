...
#variable = ['InvariantMass_of_3_particle']
sys = ['_lumi','_mu','_tau','_','_pdf','_QCD','_zzTo4L','_ZH']
systematic=['Up','Dn']
f = r.TFile("QCD_bkg0.3.root","recreate")
################################################                                                                                               
...
def buildHistDict(nbins,x_low,x_high):
    ...
    histDict[name].GetYaxis().SetLabelSize(0.5)
...
def xs_calculator(fileList = []):
   ...
   for i in range(len(variable)) :
    ...
    filesig=['Radion300_ME_controlPlotMuElectron_test.root','Radion800_ME_controlPlotMuElectron_test.root']#,'Radion400_ME_controlPlotMuElectron.root']#,'Radion800_ME_controlPlotMuElectron.root']#,'Radion900_controlPlotsys.root']    
...
######don't need to go further  below #########
      ...
      max_t = 5.*max(stackDict['s'].GetMaximum(), histDict['data_'].GetMaximum(),sigmax) ##wjets 2.,1.2*, sig 5.*
      stackDict['s'].Draw('hist H')
      stackDict['s'].GetYaxis().SetTitleSize(30);
      ...
      stackDict['s'].GetXaxis().SetTitleOffset(.01)
      stackDict['s'].GetXaxis().SetTitle('m_{#mu#mu#tau#tau} [GeV]')########33for massscan
      stackDict['s'].GetXaxis().SetTitleSize(.01)#########333 for mass scan
      ...
      hbkg1.GetYaxis().SetLabelSize(.11);
      ...
      hbkg1.GetXaxis().SetLabelSize(.11)
      ...
      c.SaveAs('%s.pdf' %pdf)      
      c.SaveAs('%s.root' %pdf)
      if l[var] == 'InvariantMass_of_4_particle_with_same_sign_part_with_totCharge==0_be4_mass_cut_in_tau1iso_tau2iso' :
