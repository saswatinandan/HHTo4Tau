  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
	    ...
	    if(tau1antiiso) {
	      double f = fakeweight(tauPt->at(vec_tau[itau]));
	      fake1 *= f/(1-f);
	      fake *= fake1;
	    }
	    
	    if(tau2antiiso) {
	      double f = fakeweight(tauPt->at(vec_tau[jtau]));
	      fake2 *= f/(1-f);
	      fake *= fake2;
	    }
	    
	    if(tau1antiisotau2antiiso) {
	      fake12 = fakeweight(tauPt->at(vec_tau[itau]))/(1-fakeweight(tauPt->at(vec_tau[itau]))) * 
		fakeweight(tauPt->at(vec_tau[jtau]))/(1-fakeweight(tauPt->at(vec_tau[jtau])));
	      fake *= fake12;
	    }
		...
		for(int totchrg=0; totchrg<2; totchrg++) {
		  ...
		  plotFill("2D_pt_distribution_of_Leading_#tau_SubLeading_#tau_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),50,0,150,50,0,50,weight);
                  plotFill("2D_pt_distribution_of_SubLeading_#tau_Leading_#tau_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",tauPt->at(vec_tau[jtau]),tauPt->at(vec_tau[itau]),50,0,150,50,0,50,weight);

		  if(tauiso == "_in_tau1antiso_tau2antiiso") {

		    plotFill("3D_#eta_distribution_of_Leading_#tau_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),tauEta->at(vec_tau[itau]),50,0,150,50,0,150,20,-2.4,2.4,weight);
                    plotFill("3D_#eta_distribution_of_Leading_#tau_with_"+totcharge[totchrg]+tauiso,tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),tauEta->at(vec_tau[itau]),50,0,150,50,0,150,20,-2.4,2.4,weight*fake);
		    plotFill("3D_#eta_distribution_of_SubLeading_#tau_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),tauEta->at(vec_tau[jtau]),50,0,150,50,0,150,20,-2.4,2.4,weight);
                    plotFill("3D_#eta_distribution_of_SubLeading_#tau_with_"+totcharge[totchrg]+tauiso,tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),tauEta->at(vec_tau[jtau]),50,0,150,50,0,150,20,-2.4,2.4,weight*fake);
		    plotFill("3D_InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso,tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),HH.M(),50,0,150,50,0,150,6,300,900,weight*leading_weight*subleading_weight*fake);
		    plotFill("3D_InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",tauPt->at(vec_tau[itau]),tauPt->at(vec_tau[jtau]),HH.M(),50,0,150,50,0,150,6,300,900,weight*leading_weight*subleading_weight);
		  ...
		  for(int i=0; i<2; i++) {
		    ...
		    for(int j=0; j<2; j++) {
		      ...
		      if(tauiso == "_in_tau1antiso_tau2antiiso") {
			plotFill("3D_InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso,pt1,pt2,H.M(),50,0,150,50,0,150,7,40,145,weight*fake*muonweight);
			plotFill("3D_InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",pt1,pt2,H.M(),50,0,150,50,0,150,7,40,145,weight*muonweight);
		      }
		    }
		  }
		}
