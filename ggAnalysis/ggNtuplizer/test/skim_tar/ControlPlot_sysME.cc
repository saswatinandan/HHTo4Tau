...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	if(Electron && M.M() >=60 && M.M() <=120 && !samesign) {
	  //	  cout << pt[0] << "\t" << eta[0] << "\t" << chargemisid(pt[0],eta[0]) << endl;
	  //cout << pt[1]<< "\t"<< eta[1] << "\t" << chargemisid(pt[1],eta[1]) << endl;
	  float chargemisidfake = chargemisid(pt[0],eta[0])*(1-chargemisid(pt[1],eta[1]));
	  chargemisidfake += chargemisid(pt[1],eta[1])*(1-chargemisid(pt[0],eta[0]));
	  //cout << chargemisidfake << endl;
	  //	  chargemisidfake /= (1-chargemisidfake);
	  //	  if(isData) chargemisidfake =1.;
	  plotFill("InvariantMass_of_"+part+"pair_with_same_sign_with_mass_60-120GeV_chargemisid",M.M(),30,60,120,weight*muonweight*chargemisidfake);
	}
	for(int i=0; i<2; i++) {
	  ...
	  for(int itau=0; itau<vec_tau.size(); itau++) {
		...
		(tau2antiiso ? "_in_tau2anti_iso" :
		 (tau1iso_tau2iso ? "_in_tau1iso_tau2iso" : "_in_tau1antiso_tau2antiiso"));
	      ...
	      int iend = (Muon || Electron) ? vec_muele.size() : musize;
	      for(int ipart=0; ipart<iend; ipart++) {
		...
		if(deltapart1t1 <0.5) continue;
		float deltapart1t2= part1.DeltaR(t2);
		if(deltapart1t2 <0.5) continue;
		
		int jstart = (Muon || Electron) ? ipart+1 : musize;
		for(int jpart=jstart; jpart<vec_muele.size(); jpart++) {
		  ...
		  if(deltapart1part2 <0.5) continue;
		  float deltapart2t1= part2.DeltaR(t1);
		  if(deltapart2t1 <0.5) continue;
		  float deltapart2t2 = part2.DeltaR(t2);
		   if(deltapart2t2 <0.5) continue;
		  ...
		  if(Electron && !samesign) {
		    chargemisidfake  = chargemisid(pt[ipart],eta[ipart]);
		    chargemisidfake *= chargemisid(pt[jpart],eta[jpart]);
		    chargemisidfake /= (1-chargemisidfake);
		  }
		  bool taupartCharge;
		  ...
		  for(int partchrg=0; partchrg<2; partchrg++) {
			...
			if(Electron && !samesign && tau1iso_tau2iso) plotFill("InvariantMass_of_4_particle_with_"+title+"_be4_mass_cut"+tauiso+"_chargemisid",HH.M(),9,100,1000,weight*leading_weight*subleading_weight*chargemisidfake);
			...
			if(tau1iso_tau2iso && partcharge[1] && taupartcharge_[0]) {
