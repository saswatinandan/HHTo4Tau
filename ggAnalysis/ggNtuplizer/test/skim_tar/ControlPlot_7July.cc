...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
    ...
    output.erase(root);
    output += "_controlPlot_11July_oppositesignmuon.root";
    //output = ".root";
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
		...
		for(int totchrg=0; totchrg<2; totchrg++) {
		  ...
		  plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",HH.M(),7,250,900,weight*leading_weight*subleading_weight);
		  ...
		  //                  plotFill("InvariantMass_of_#tau_pair_with_"+totcharge[totchrg]+tauiso,T.M(),50,50,150,weight*fake);
		  plotFill("InvariantMass_of_#tau_pair_with_"+totcharge[totchrg]+tauiso,T.M(),35,50,120,weight*fake);
		  //		  plotFill("InvariantMass_of_#tau_pair_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",T.M(),50,50,150,weight);
		  ...
		  for(int i=0; i<2; i++) {
		    ...
		    for(int j=0; j<2; j++) {
		      ...
		      //		      plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso,H.M(),16,40,200,weight*fake*muonweight);
		      plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso,H.M(),11,40,150,weight*fake*muonweight);
		      //		      plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",H.M(),16,40,200,weight*muonweight);
		      plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",H.M(),11,40,150,weight*muonweight);
		    }
		  }
		}
	      }
