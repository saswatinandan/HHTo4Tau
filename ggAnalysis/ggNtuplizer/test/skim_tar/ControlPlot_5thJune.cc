...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	if(tauPt->at(itau) > 20 && fabs(tauEta->at(itau)) < 2.3 && antielemu && taupfTausDiscriminationByDecayModeFinding->at(itau) !=0 
      ...
      if(Muon) {
	charge = muCharge->at(vec_muele[0])*muCharge->at(vec_muele[1]) >0;
      }
      else {
	charge = eleCharge->at(vec_muele[0])*eleCharge->at(vec_muele[1]) >0;
      }
	...
	m1.SetPtEtaPhiE(elePt->at(vec_muele[0]),eleEta->at(vec_muele[0]),elePhi->at(vec_muele[0]),eleEn->at(vec_muele[0]));
      ...
      if(Muon) {
	part = "#mu_";
	pt1 = muPt->at(vec_muele[0]);
	pt2 = muPt->at(vec_muele[1]);
	eta1 = muEta->at(vec_muele[0]);
	eta2 = muEta->at(vec_muele[1]);
      }
      else {
	part = "#ele_";
	pt1 = elePt->at(vec_muele[0]);
	pt2 = elePt->at(vec_muele[1]);
	eta1 = eleEta->at(vec_muele[0]);
	eta2 = eleEta->at(vec_muele[1]);
      }
      //      cout << phi1 << "\t" << phi2 << endl;
      ...
      if(!isData && Muon) {
	leading_weight = scaleFactor(pt1,eta1);
	subleading_weight = scaleFactor(pt2,eta2);
	...
	if(M.M() >=20 && M.M() <=200) {
	  ...
	  plotFill("pt_distribution_of_Leading_"+part+title+"_with_mass_20-200GeV",pt1,50,0,100,leading_weight);
          plotFill("pt_distribution_of_SubLeading_"+part+title+"_with_mass_20-200GeV",pt2,50,0,100,subleading_weight);
          plotFill("eta_distribution_of_Leading_"+part+title+"_with_mass_20-200GeV",eta1,20,-2.4,2.4,leading_weight);
          plotFill("eta_distribution_of_SubLeading_"+part+title+"_with_mass_20-200GeV",eta2,20,-2.4,2.4,subleading_weight);
        }
	
	if(M.M() >=60 && M.M() <=120) {
	  plotFill("InvariantMass_of_"+part+"pair_with_" +title+"_with_mass_60-120GeV",M.M(),30,60,120,muonweight);
	  plotFill("pt_distribution_of_Leading_"+part+title+"_with_mass_60-120GeV",pt1,50,0,100,leading_weight);
	  plotFill("pt_distribution_of_SubLeading_"+part+title+"_with_mass_60-120GeV",pt2,50,0,100,subleading_weight);
	  plotFill("eta_distribution_of_Leading_"+part+title+"_with_mass_60-120GeV",eta1,20,-2.4,2.4,leading_weight);
	  plotFill("eta_distribution_of_SubLeading_"+part+title+"_with_mass_60-120GeV",eta2,20,-2.4,2.4,subleading_weight);
	}
	for(int j=0; j<3; j++ ){
	  if(!jet[j]) continue;
	...
	bool taumuCharge;
	if(Muon) {
	  taumuCharge = (muCharge->at(vec_muele[0])+muCharge->at(vec_muele[1]) + tauCharge->at(vec_tau[0])+tauCharge->at(vec_tau[1])) ==0;
	}
	else {
	  taumuCharge = (eleCharge->at(vec_muele[0])+eleCharge->at(vec_muele[1]) + tauCharge->at(vec_tau[0])+tauCharge->at(vec_tau[1])) ==0;
	}
	...
	for(int i=0; i<2; i++) {
	  ...
	  plotFill("InvariantMass_of_4_particle_with_"+totcharge[i],HH.M(),7,250,900,muonweight);
	}
	if(taumucharge_[0]) {
	  bool muCharge_;
	  if(Muon) {
	    muCharge_ = (muCharge->at(vec_muele[0])+muCharge->at(vec_muele[1])) ==0;
	  }
	  else {
	    muCharge_ = (eleCharge->at(vec_muele[0])+eleCharge->at(vec_muele[1])) ==0;
          }
	  ...
	  for(int i=0; i<2; i++) {
	    ...
	    plotFill("InvariantMass_of_4_particle_with_"+taumucharge[i],HH.M(),7,250,900,muonweight);
	  }
	}
	bool taumucharge;
	for(int i=0; i<2; i++) {
	  for(int j=0; j<2; j++) {
	    ...
	    if(Muon) {
	      taumucharge = tauCharge->at(vec_tau[i]) + muCharge->at(vec_muele[j]) ==0;
	    }
	    else {
	      taumucharge = tauCharge->at(vec_tau[i]) + eleCharge->at(vec_muele[j]) ==0;
            }
	    ...
	    if(Muon) {
	      m.SetPtEtaPhiE(muPt->at(vec_muele[j]),muEta->at(vec_muele[j]),muPhi->at(vec_muele[j]),muEn->at(vec_muele[j]));
	    }
	    else {
	      m.SetPtEtaPhiE(elePt->at(vec_muele[j]),eleEta->at(vec_muele[j]),elePhi->at(vec_muele[j]),eleEn->at(vec_muele[j]));
            }
	    H= t+m;
