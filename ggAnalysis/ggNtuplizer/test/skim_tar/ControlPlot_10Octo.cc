  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	if(vec_tau.size() >1) {
	  ...
	  for(int itau=0; itau<vec_tau.size(); itau++) {
	    ...
	    double Pt1 = (isys==0) ?  tauPt->at(vec_tau[itau]) : ((isys==1) ? tauPt->at(vec_tau[itau])*tauESUp : tauPt->at(vec_tau[itau])*tauESDn);
	    double e1 = (isys==0) ?  tauEnergy->at(vec_tau[itau]) : ((isys==1) ? tauEnergy->at(vec_tau[itau])*tauESUp : tauEnergy->at(vec_tau[itau])*tauESDn);
	    t1.SetPtEtaPhiE(Pt1,tauEta->at(vec_tau[itau]),tauPhi->at(vec_tau[itau]),e1);
	    
	    for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {

	      double Pt2 = (isys==0) ?  tauPt->at(vec_tau[jtau]) :((isys==1) ? tauPt->at(vec_tau[jtau])*tauESUp : tauPt->at(vec_tau[jtau])*tauESDn);
	      double e2 = (isys==0) ?  tauEnergy->at(vec_tau[jtau]) :((isys==1) ? tauEnergy->at(vec_tau[jtau])*tauESUp : tauEnergy->at(vec_tau[jtau])*tauESDn);
		...
		for(int jmu=imu+1; jmu<vec_muele.size(); jmu++) {
		  ...
		  for(int totchrg=0; totchrg<2; totchrg++) {
		    ...
		    if(isys ==0) {
		      ...
		      TLorentzVector taugen_;
		      std::vector<TLoretzVector> taugen;
		      for(int imc=0; imc<nMC; imc++) {
			if(mcHadronPt->at(imc) < 0) continue;
			  taugen_.SetPtEtaPhiE(mcHadronPt->at(imu),mcHadronEta->at(imu),mcHadronPhi->at(imu),mcHadronE->at(imu));
			  taugen.push_back(taugen_);
		      }
		      double deltar1(999), deltar2(999);

		      for(unsigned int i=0; i<taugen.size(); taugen++) {
			deltar1 = dR_(t1.Pt(),t1.Phi(),taugen[i].Pt(),taugen[i].Phi());
			if (deltaR1 <0.5) break;
		      }
		      for(unsigned int i=0; i<taugen.size(); taugen++) { 
			deltar2 = dR_(t1.Pt(),t1.Phi(),taugen[i].Pt(),taugen[i].Phi());
			if (deltaR2 <0.5) break; 
		      }
		      if(deltar1 <0.5 && deltar2 <0.5) {
			for(int isys=0; isys <3; isys++) {
			  string sys = (isys==0) ? "Nor" : ((isys==1) ? "Up" : "Dn");
			  TLorentzVector tausys1,tausys2;
			  tausys1.SetPtEtaphiE(t1.Pt()
