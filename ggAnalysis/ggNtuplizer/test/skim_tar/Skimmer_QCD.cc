...
void Skimmer::Loop(TString outputName, bool Muon, int skm)
{
  ...
  for (int jentry=0; jentry<nentries;jentry++) {
	...
	if(imu == 0 && muPt->at(imu) <18) {
	  //	  cout << "imu== " << imu << "\t" << "break due to <18" << endl;
	  break;
	}
	//cout << "imu== " << imu << endl;
	...
	if(mupt && id && fabs(muEta->at(imu)) <2.4) {
	  firstPart = false;
	  isMuEle +=1;
	}
	if(isMuEle ==3) break;
...
int main(int argc, char* argv[]){
  ...
  Skimmer t("root://cmseos.fnal.gov//store/user/snandan/Data19"+FinaName);
  cout << "outname== " << "root://cmsxrootd.fnal.gov//store/user/snandan/Data19" << FinaName << endl;
