...
int  main(int argc, char** argv) {
  ...
  std::string output = "chargemisid1.root";
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	//	  cout << "lumi=========" << LumiWeight << input[k] << endl;
      ...
      bool bl1(false),bl2(false),bh1(false),bh2(false),bm1(false),bm2(false),eh1(false),eh2(false),el1(false),el2(false),em1(false),em2(false), genele1(false), genele2(false), masscut(false);
      int e1indx(-1), e2indx(-1);
      
      for(int isys=0; isys<3; isys++) {
	float scale = (isys==0) ? 1. : ((isys==1) ? 1.02 : 0.98);
	string sys = (isys==0) ? "Nor" : ((isys==1) ? "Up" : "Dn");

	for (int iele = 0; iele < nEle; ++iele) {

	  bl1 = bh1 = bm1 = eh1 = el1 = em1 = genele1 = false;
	  double elept = elePt->at(iele)*scale;
	  if((elept <=24) || (fabs(eleEta->at(iele)) >=2.5 )) continue;
	  ...
	  for(int imc=0; imc<nMC; imc++) {
	    ...
	    float deltapt = (fabs(mcPt->at(imc)*scale-elept)/mcPt->at(imc)*scale);
	  ...
	  else if(elept >=25 && elePt->at(iele) <50) {
	    if(fabs(eleEta->at(iele)) <=1.479) {
	      bm1 = true;
            }
            else if(fabs(eleEta->at(iele)) >1.479 && fabs(eleEta->at(iele)) <2.5) {
	...
	if(muPt->at(imu) >9 && fabs(muEta->at(imu)) <2.4 && id && IsoMu <0.3) {
      ...
      string bkg = (!genele1 || !genele2) ? ((sample.find("DY") != string::npos) ? "DY" : ((sample.find("ST") != string::npos) ? "ST" : sample)) : "" ;
      std::string type = (isData) ? "Data" : ((genele1 && genele2) ? ((sample.find("DY") != string::npos) ? "DY_Zee" : sample+"_Zee") : bkg) ;
      
      std::string title = (samesign) ? "_SS" : "_OS";

      float chargemisidfake(1.);
      if(!isData) {
        chargemisidfake = chargemisid(elePt->at(e1indx),eleEta->at(e1indx))/(1-chargemisid(elePt->at(e1indx),eleEta->at(e1indx)));
	//cout << "pt1= " << elePt->at(e1indx) << "\t" << "eta1= " << eleEta->at(e1indx) ;
	chargemisidfake += chargemisid(elePt->at(e2indx),eleEta->at(e2indx))/(1-chargemisid(elePt->at(e2indx),eleEta->at(e2indx)));
	...
	if(genele1 && genele2 && title.find("OS") != string::npos) {
	  ...
	  plotFill("BB_LL"+title,type+"_chargemisid",M.M(),30,60,120,weight);
	}
      }
      if((bl1 && bm2) || (bm1 && bl2)) {
	...
	if(genele1 && genele2  && title.find("OS") != string::npos) {
	  weight *= chargemisidfake;
	  plotFill("EE_LL"+title,type+"_chargemisid",M.M(),30,60,120,weight);
	}
      }
