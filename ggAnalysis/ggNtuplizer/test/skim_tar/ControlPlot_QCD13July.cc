...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
    ...
    double f1(0), f2(0);
    TH1F* h1 = new TH1F("h","h",30,50,200);
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
	    ...
	    if(tau1antiisotau2antiiso) {
	      fake1 = fakeweight(tauPt->at(vec_tau[itau]))/(1-fakeweight(tauPt->at(vec_tau[itau])));
	      fake2 = fakeweight(tauPt->at(vec_tau[jtau]))/(1-fakeweight(tauPt->at(vec_tau[jtau])));
	      fake *= fake1*fake2;
	    }
	    ...
	    for(int imu=0; imu<vec_muele.size(); imu++) {
	      ...
	      HH = T+m1;
	      //weight = fake =1;
	      plotFill("InvariantMass_of_3_particle"+tauiso,HH.M(),30,50,200,weight*fake);
	      plotFill("InvariantMass_of_3_particle_with_"+tauiso+"_nofake_weight",HH.M(),30,50,200,weight);
	      /*if(tau2antiiso) cout << "2..." << weight << "\t" << GetGenWeight << "\t" << LumiWeight << "\t" << HH.M() << endl;
	      if(tau2antiiso) cout << "1..." << weight << "\t" << GetGenWeight << "\t" << LumiWeight << "\t" << HH.M() << endl;
	      if(tau1iso_tau2iso) cout << "12..." << weight << "\t" << GetGenWeight << "\t" << LumiWeight << "\t" << HH.M() << endl;*/
	      /*	      if(fake==1) {
		cout << weight << "\t" << GetGenWeight << "\t" << LumiWeight << "\t" << HH.M() << endl;
		h1->Fill(HH.M(),weight);
		}*/
	      TLorentzVector H;
	      for(int i=0; i<2; i++) {
		...
		H = tau+m1;
		plotFill("InvariantMass_of_taumu_with_"+tauiso,H.M(),50,50,150,weight*fake);
		plotFill("InvariantMass_of_taumu_with_"+tauiso+"_nofake_weight",H.M(),50,50,150,weight);
	      }
	    }
	  }
	}
    ...
    fout->cd();
    //    for(int i=0; i<h1->GetNbinsX();i++) cout << h1->GetBinContent(i+1) << "\t" << "E" << i+1 << "\t" << h1->GetBinError(i+1) << endl;
...
double fakeweight(double pt)  {
  ...
  //return .002+.153*TMath::Exp(-.085*pt);//from QCD
  return .016+.632*TMath::Exp(-.100*pt);
  //return .017+.165*TMath::Exp(-.060*pt); //from DY
