...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	for (int iele = 0; iele < nEle; ++iele) {
      ...
      else { ////////////////// will be implemented later
      }
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
	    ...
	    bool tau1antiiso = (tauByLooseIsolationMVArun2v1DBoldDMwLT->at(vec_tau[itau]) ==0 && //////////////////// 0 means antiisolated , !=0 means isolated
