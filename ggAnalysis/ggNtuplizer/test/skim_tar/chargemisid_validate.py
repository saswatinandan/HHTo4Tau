...
histDict = {}
variable = ['InvariantMass_of_#ele_pair_with_same_sign_with_mass_60-120GeV_chargemisid']
...
#      ifile = r.TFile('chargemisid.root')
      ...
      for iFileName, iFileLocation in fileList:
         ...
         print var,iFileName
         if ifile.Get(var) :
            FillHisto(ifile.Get(var), histDict[iFileName])

      filesig = r.TFile('Data_EE_controlPlotElectron_Sir.root')
      hsig = filesig.Get('InvariantMass_of_#ele_pair_with_same_sign_and_no_cut_on_#tau_leg_with_mass_60-120GeV')
      print hsig,filesig
      if hsig : histDict['data_'] = hsig
...
#      pad1.SetLogy()
      ...
      stackDict['s'].SetTitle(';%s;Events' %(var))
...
fileList =  [#('data_', '/eos/uscms/store/user/snandan/Data19_controlPlot.root'),
            ('ZZTo4L', '%s/zzTo4L_EE_controlPlotElectron_Sir.root' %dirName),
            ('WW', '%s/WW_EE_controlPlotElectron_Sir.root' %dirName),
            ('t#bar{t}+jets', '%s/TT_EE_controlPlotElectron_Sir.root' %dirName),
            ('WJets', '%s/WJets_controlPlotElectronSir.root' %dirName),
            ('DY', '%s/DY_controlPlotElectronSir.root' %dirName),
            ('WZ', '%s/WZ_EE_controlPlotElectron_Sir.root' %dirName),
            ('ZZ', '%s/ZZ_EE_controlPlotElectron_Sir.root' %dirName),
             ('ZH', '%s/ZH_EE_controlPlotElectron_Sir.root' %dirName),
            ('Single Top', '%s/ST_controlPlotElectronSir.root' %dirName)]
