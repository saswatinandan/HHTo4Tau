...
std::map<string, map<string, TH1F* >* >* myMap1;

void plotFill(std::string name1, std::string name2, float x, int nx,
	      float nxmin, float nxmax, double weight=1) {

  if (myMap1->find(name1) == myMap1->end()) {
    TH1F* h = new TH1F(name2.c_str(), name2.c_str(), nx, nxmin, nxmax);
    map< string, TH1F*>* mapval = new map< string, TH1F*>();
    h->SetDefaultSumw2();
    (*mapval)[name2] = h;
    (*myMap1)[name1] = mapval;
  }

  if((*myMap1)[name1]->find(name2) == (*myMap1)[name1]->end()) {
    TH1F* h = new TH1F(name2.c_str(), name2.c_str(), nx, nxmin, nxmax);
    (*(*myMap1)[name1])[name2] = h;
  }
  (*(*myMap1)[name1])[name2]->SetDefaultSumw2();
  (*(*myMap1)[name1])[name2]->Fill(x,weight);
}


  TH1F* nplot1(std::string name1, std::string name2) {
    if (myMap1->find(name1) != myMap1->end() && (*myMap1)[name1]->find(name2) != (*myMap1)[name1]->end()) {
    return (*(*myMap1)[name1])[name2];
    }
  else
    return 0;
}
