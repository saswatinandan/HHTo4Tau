...
def xs_calculator(fileList = []):
    ...
    for var in range(len(l)) :
      for iFileName, iFileLocation in fileList:
         ...
         if ifile_.Get(l[var]) :
            ...
            print 'x_low= ', x_low, x_high, nbins
      ...
      hist12_data.Sumw2()
      hist1_Mc = r.TH2F('tau1_Mc', '', nbins, int(x_low),int(x_high), nbins, int(x_low),int(x_high))
      hist1_Mc.Sumw2()
   ...
   #loop over all the samples                                                                                                                 
      ...
      if l[var] == variable[i] +'_in_tau1iso_tau2iso_nofake_weight' :
          ...
          for xbin in range(hist1_data.GetNbinsX()) :
              pt = hist1_data.GetXaxis().GetBinCenter(xbin+1)
              fake = fakeweight(pt)
              for ybin in range(hist1_data.GetNbinsY()) :
                  bincont = hist1_data.GetBinContent(xbin+1,ybin+1)
#                  hist1_data.SetBinError(xbin+1,ybin+1,0)
                  hist1_data.SetBinContent(xbin+1,ybin+1,bincont*fake/(1-fake))
                  hist1_data.SetBinError(xbin+1,ybin+1,r.TMath.Sqrt(bincont*fake/(1-fake)))

          for ybin in range(hist2_data.GetNbinsY()) :
              pt = hist2_data.GetYaxis().GetBinCenter(ybin+1)
              fake = fakeweight(pt)
              for xbin in range(hist2_data.GetNbinsX()) :
                  bincont = hist2_data.GetBinContent(xbin+1,ybin+1)
                  #hist2_data.SetBinError(xbin+1,ybin+1,0)
                  hist2_data.SetBinContent(xbin+1,ybin+1,bincont*fake/(1-fake))
                  hist2_data.SetBinError(xbin+1,ybin+1,r.TMath.Sqrt(bincont*fake/(1-fake)))

          for xbin in range(hist12_data.GetNbinsX()):
              pt1 = hist12_data.GetXaxis().GetBinCenter(xbin+1)
              fake1 = fakeweight(pt1)
              for ybin in range(hist12_data.GetNbinsY()) :
                  pt2 = hist12_data.GetYaxis().GetBinCenter(ybin+1)
                  fake2 = fakeweight(pt2)
                  bincont = hist12_data.GetBinContent(xbin+1,ybin+1)
          #        hist12_data.SetBinError(xbin+1,ybin+1,0)
                  hist12_data.SetBinContent(xbin+1,ybin+1,bincont*fake1/(1-fake1)*fake2/(1-fake2))
                  hist12_data.SetBinError(xbin+1,ybin+1,r.TMath.Sqrt(bincont*fake1/(1-fake1)*fake2/(1-fake2)))
