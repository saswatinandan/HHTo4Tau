...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
    ...
    //      for ( Int_t i = 200000; i < ; i++) { 
      ...
      bool PassTrigger =   (Muon) ? ((HLTEleMuX >> 19 & 1) || (HLTEleMuX >> 20 & 1)) : ( Electron ? (HLTEleMuX >> 5 & 1) : (HLTEleMuX >> 5 & 1));// || (HLTEleMuX >> 19 & 1) || (HLTEleMuX >> 20 & 1)) : (HLTEleMuX >> 34 & 1);
	...
	for(int itau=0; itau<vec_tau.size(); itau++) {
	  ...
	  for(int jtau=itau+1; jtau<vec_tau.size(); jtau++) {
		...
		H = tau+m1;
		plotFill("InvariantMass_of_taumu"+tauiso,H.M(),18,20,150,weight*fake);
