...
int  main(int argc, char** argv) {
  ...
  for(int k=2; k<input.size(); k++ ) {
    ...
    output = input[1] + "Fakeintest_" + output;
...
void TT_fake(TH1F* h_deno, TH1F* h_neu, TTree* Run_Tree, bool Muon, std::string input, TH1F* HistoTot )       {
  ...
  for ( Int_t i = 0; i < nentries_wtn; i++) {
    ...
    bool PassTrigger =   (Muon) ? ((HLTEleMuX >> 14 & 1) || (HLTEleMuX >> 15 & 1)) : (HLTEleMuX >> 33 & 1);// || (HLTEleMuX >> 19 & 1) || (HLTEleMuX >> 20 & 1)) : (HLTEleMuX >> 34 & 1);
...
void Wjets_fake(TH1F* h_deno, TH1F* h_neu, TTree* Run_Tree, bool Muon, std::string input, TH1F* HistoTot )       {
  ...
  HistoPUMC->Scale(1.0/HistoPUMC->Integral());
    ...
    if (i % 1000 == 0) fprintf(stdout, "\r  Processed events: %8d of %8d ", i, nentries_wtn);
    bool PassTrigger =   (Muon) ? ((HLTEleMuX >> 19 & 1) || (HLTEleMuX >> 20 & 1))  : (HLTEleMuX >> 34 & 1);// || (HLTEleMuX >> 19 & 1) || (HLTEleMuX >> 20 & 1)) : (HLTEleMuX >> 34 & 1);                                                                                                    
    if(!PassTrigger) continue;
    int count_bjet(0);
    for (int ijet= 0 ; ijet < nJet ; ijet++){
      if (jetPt->at(ijet) > 20 && fabs(jetEta->at(ijet)) < 2.5 && jetCSV2BJetTags->at(ijet) > 0.460){
	count_bjet +=1;
	break;
      }
    }
