...
int  main(int argc, char** argv) {
  ...
  for(int k=2; k<input.size(); k++ ) {
    ...
    output = "fakecalculate_withnogenweight_in"+input[k];
	...
	//	if(chrg[0]+chrg[1]+tauCharge->at(vec_tau[0])+tauCharge->at(vec_tau[1]) !=0) continue;
	...
	std::string title = (tau1antiiso) ? "tau1anti_iso" : 
	  (tau2antiiso ? "tau2anti_iso" : 
	   (tau1iso_tau2iso ? "tau1iso_tau2iso" : "tau1antiso_tau2antiiso"));
	...
	T= t1+t2;
	HH = T+M;
	float taupt1 = tauPt->at(vec_tau[0]);
	float taupt2 =tauPt->at(vec_tau[1]);
	
	plotFill("InvariantMass_of_tau_pair_with_tau_same_sign_muon_opposite_sign_in_"+title,T.M(),6,50,110,fake);
	plotFill("pt_distribution_of_Leading_#tau_with_tau_same_sign_muon_opposite_sign_in_"+title,tauPt->at(vec_tau[0]),130,20,150,fake);
	plotFill("pt_distribution_of_SubLeading_#tau_with_tau_same_sign_muon_opposite_sign_in_"+title,tauPt->at(vec_tau[1]),130,20,150,fake);
	plotFill("InvariantMass_of_4_particle_with_tau_same_sign_muon_opposite_sign_in_"+title,HH.M(),7,250,900,fake);
	plotFill("InvariantMass_of_"+part+"pair_with_mass_60-120GeV_with_tau_same_sign_muon_opposite_sign_in_"+title,M.M(),30,60,120,fake);
	//	plotFill("pt_distribution_of_Leading_"+part+"_with_mass_60-120GeV_with_tau_same_sign_muon_opposite_sign_in_"+title,50,0,100,fake);
