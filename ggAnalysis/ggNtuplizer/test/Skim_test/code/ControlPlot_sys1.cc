  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	if(vec_tau.size() >1) {
	  ...
	  for(int itau=0; itau<vec_tau.size(); itau++) {
		...
		if(m1.DeltaR(t1) <0.1) continue;
		if(m1.DeltaR(t2) <0.1) continue;
		bool mu1match(false);
		...
		for(int jmu=imu+1; jmu<vec_muele.size(); jmu++) {
		  ...
		  if(m2.DeltaR(m1) <0.1) continue;
		  if(m2.DeltaR(t1) <0.1) continue;
		  if(m2.DeltaR(t2) <0.1) continue;
		  ...
		  /*		  if(isys ==0) {
		    for(int muchrg=0; muchrg<2; muchrg++) {
		      ...
		      plotFill("InvariantMass_of_4_particle_with_"+title+tauiso,HH.M(),7,250,1000,weight*leading_weight*subleading_weight*fake);
		  ...
		  if(isys ==0) {
                    for(int muchrg=0; muchrg<2; muchrg++) {
                      //                      if(muchrg!=1) continue;                                                                                                                                        
		      if(!mucharge[muchrg]) continue;
		      if(!taumucharge_[muchrg]) continue;
		      std::string title = muCharge_[muchrg]+totcharge[muchrg];
                      plotFill("InvariantMass_of_tau_pair_with_"+title+tauiso,T.M(),6,50,110,weight*fake);
                      plotFill("pt_distribution_of_Leading_#tau_with_"+title+tauiso,tauPt->at(vec_tau[itau]),50,0,150,weight*fake);
                      plotFill("pt_distribution_of_SubLeading_#tau_with_"+title+tauiso,tauPt->at(vec_tau[jtau]),50,0,150,weight*fake);
                      plotFill("eta_distribution_of_Leading_#tau_with_"+title+tauiso,tauEta->at(vec_tau[itau]),20,-2.4,2.4,weight*fake);
		  ...
		  for(int totchrg=0; totchrg<2; totchrg++) {
		    ...
		    if(tau1match && tau2match && tau1iso_tau2iso) {
		      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_"+systematic,HH.M(),7,250,1000,weight*leading_weight*subleading_weight*fake);
		    }
		    if(isys ==0) {
		      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso,HH.M(),9,100,1000,weight*leading_weight*subleading_weight*fake);
		      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",HH.M(),9,100,1000,weight*leading_weight*subleading_weight);
