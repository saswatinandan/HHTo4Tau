...
int  main(int argc, char** argv) {
  ...
  TH1F* h_counter   = new TH1F("counter","counter",32,0.,32);
  ...
  TH1F* h_mu1pt   = new TH1F("mu1Pt","",25,0.,50.);
  TH1F* h_mu2pt   = new TH1F("mu2Pt","",25,0.,50.);
  TH1F* h_tau1pt   = new TH1F("tau1Pt","",50,0.,100.);
  TH1F* h_tau2pt   = new TH1F("tau2Pt","",50,0.,100.);
  ...
  //  std::string key[31] = {"Event","genMuon","mPt","mEta","Isom","isMediumMuon","genMuon","dR","mPt","mEta","Isom","isMediumMuon","genTau","dR","tauPt","tauEta","taueleRejection","taumuonRejection","tauIso","tauDecayMode","genTau","dR","tauPt","tauEta","taueleRejection","taumuonRejection","tauIso","tauDecayMode","taucharge","muoncharge","taumuoncharge"};
  std::string key[27] = {"Event","mPt","mEta","Isom","isMediumMuon","dR","mPt","mEta","Isom","isMediumMuon","dR","tauPt","tauEta","taueleRejection","taumuonRejection","tauDecayMode","tauIso","dR","tauPt","tauEta","taueleRejection","taumuonRejection","tauDecayMode","tauIso","taucharge","muoncharge","taumuoncharge"}; 
  for (int i=0; i<27; i++) h_counter->GetXaxis()->SetBinLabel(i+1,key[i].c_str());
  ...
  //  for ( Int_t i = 0; i < 1000000; i++) { 
    ...
    fflush(stdout);
    float PUWeight = 1;
    bool PassTrigger =   (HLTEleMuX >> 15 & 1) == 1;
    ...
    int donotTake(0), mucharge(0),tauhad(0),tauele(0),taumu(0);
	...
	if(fabs(mcMomPID->at(imc)) ==23)  nele +=1;
      }
      if(fabs(mcPID->at(imc)) ==13) {
        if(fabs(mcMomPID->at(imc)) ==23)  nmu +=1;
      }
	...
	if(fabs(mcPID->at(imc)) ==16 && fabs(mcMomPID->at(imc)) ==15 && mcGMomPID->at(imc) ==23) ntauh +=1;
    ...
    if(ntau==4) {
      if(ntaumu==2 && ntauele==0 && ntauh==4) h_taucount->Fill(0);
      if(ntaumu==2 && ntauele==1&& ntauh==4) h_taucount->Fill(1);
      if(ntaumu ==1 && ntauele==1&&ntauh==4) h_taucount->Fill(2);
      if(ntaumu ==1 && ntauele==2&&ntauh==4) h_taucount->Fill(3);
      if(ntaumu ==3 && ntauele==0 && ntauh==4) h_taucount->Fill(4);
      if(ntaumu==1 && ntauele==0 && ntauh==4) h_taucount->Fill(5);
      ...
      if(ntauele==2 && ntaumu==0&&ntauh==4) h_taucount->Fill(9);
    ...
    /*    for(int imc=0; imc<nMC; imc++) {
      if(fabs(mcPID->at(imc)) ==13 && fabs(mcMomPID->at(imc)) ==15 && fabs(mcGMomPID->at(imc)) ==23) {
	TLorentzVector genMu;
	genMu.SetPtEtaPhiE(mcPt->at(imc),mcEta->at(imc),mcPhi->at(imc),mcE->at(imc));
	genmu.push_back(genMu);
      }

      if(mcHadronPt->at(imc) >0 || mcHadronPt->at(imc) ==0) {
	TLorentzVector genTauh;
        genTauh.SetPtEtaPhiE(mcHadronPt->at(imc),mcHadronEta->at(imc),mcHadronPhi->at(imc),mcHadronE->at(imc));
	gentauhadron.push_back(genTauh);
      }
     }

    if( gentauhadron.size() !=2) cout << "not ==2" << endl;
    ...
    for  (int imu=0 ; imu < nMu; imu++){
	 ...
	 if(fabs(mcPID->at(imc)) ==13 && fabs(mcMomPID->at(imc)) ==15 && fabs(mcGMomPID->at(imc)) ==23) {
       ...
       if(!trueMu) continue;
       h_counter->Fill(1);*/
       if(muPt->at(imu) <= 18) continue;
       h_counter->Fill(1);
       if(fabs(muEta->at(imu)) >= 2.4) continue;
       h_counter->Fill(2);
	 ...
	 if(!trueMu) continue;
	 h_counter->Fill(6);*/
	 if(reco1mu.DeltaR(reco2mu) <0.5) continue;
	 h_counter->Fill(5);
	 if(muPt->at(jmu) <= 9) continue;
         h_counter->Fill(6);
	 if(fabs(muEta->at(jmu)) >= 2.4) continue;
	 h_counter->Fill(7);
	 ...
	 for  (int itau=0 ; itau < nTau; itau++){
	   ...
	   /*	   for(int imc=0; imc<nMC; imc++) {
	     if(fabs(mcPID->at(imc)) ==15 && fabs(mcMomPID->at(imc)) ==23) {
	       TLorentzVector gentau;
	       gentau.SetPtEtaPhiE(mcPt->at(imc),mcEta->at(imc),mcPhi->at(imc),mcE->at(imc));
	       if(gentau.DeltaR(reco1tau) <0.5) { truetau = true; break;}
	     }
	   }
	   if(!truetau) continue;
	   h_counter->Fill(12);*/
	   ...
	   if(reco1tau.DeltaR(reco1mu) <0.5 || reco1tau.DeltaR(reco2mu) <0.5) continue;
	   h_counter->Fill(10);
	   ...
	   for  (int jtau=itau+1 ; jtau < nTau; jtau++){
		...
		TLorentzVector gentau;
		gentau.SetPtEtaPhiE(mcPt->at(imc),mcEta->at(imc),mcPhi->at(imc),mcE->at(imc));
		if(gentau.DeltaR(reco2tau) <0.5) { truetau = true; break;}
	       }
	     }
	     if(!truetau) continue;
	     h_counter->Fill(20);*/
	     ...
	     //h_counter->Fill(21);
	     if(reco1tau.DeltaR(reco2tau) <0.5 || reco2tau.DeltaR(reco2mu) <0.5 || reco2tau.DeltaR(reco1mu) <0.5) continue;
	     h_counter->Fill(17);
	     if(tauPt->at(jtau) < 20) continue;
	     h_counter->Fill(18);
	     if(fabs(tauEta->at(jtau)) > 2.3) continue;
	     h_counter->Fill(19);
	     if(tauByMVA6LooseElectronRejection->at(jtau) ==0) continue;
	     h_counter->Fill(20);
	     if(tauByTightMuonRejection3->at(jtau) ==0) continue;
	     h_counter->Fill(21);
	     //tauByoldDecayModeFind    if(tauByLooseCombinedIsolationDeltaBetaCorr3Hits->at(jtau) ==0) continue;
	     ...
	     h_counter->Fill(25);
	     if(muCharge->at(imu)*tauCharge->at(jtau) >0) continue;
	     h_counter->Fill(26);
	     found = true;
	     break;
	   }
