...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	vec_tau = (isys==0 || isys==1) ? vec_tauNor : ((isys==2) ? vec_tauUp : vec_tauDn);
	std::string systematic = (isys==0) ? "" : ((isys==1) ? "Nom" : ((isys==2) ? "Up" : "Dn"));
	  ...
	  for(int itau=0; itau<vec_tau.size(); itau++) {
		...
		for(int jmu=imu+1; jmu<vec_muele.size(); jmu++) {
		  ...
		  if(isys ==0) {
		    for(int muchrg=0; muchrg<2; muchrg++) {
		      ...
		      plotFill("InvariantMass_of_mu_pair_with_"+title+tauiso,M.M(),6,60,120,weight*fake);
                      plotFill("pt_distribution_of_Leading_#mu_with_"+title+tauiso,muPt->at(vec_muele[imu]),50,0,150,weight*fake);
                      plotFill("pt_distribution_of_SubLeading_#mu_with_"+title+tauiso,muPt->at(vec_muele[jmu]),50,0,150,weight*fake);
		      ...
		      plotFill("InvariantMass_of_4_particle_with_"+title+tauiso,HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake);
		    }
		  }
		  ...
		  for(int totchrg=0; totchrg<2; totchrg++) {
			...
			if(mu1match || mu2match) {
			plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_muSFup",HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake*muSFup);
			plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_muSFdn",HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake*muSFdn);
			}
			if(taugen.size() !=0) {
                          for(unsigned int igentau=0; igentau<taugen.size(); igentau++) {
			...
			if(tau1match || tau2match) {
			plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_tauSFup",HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake*tauSFup);
			plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_tauSFdn",HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake*tauSFdn);
			}
		      }
		      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso,HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake);
                      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",HH.M(),6,250,900,weight*leading_weight*subleading_weight);
		      ...
		      plotFill("pt_distribution_of_Leading_#mu_with_"+totcharge[totchrg]+tauiso,muPt->at(vec_muele[imu]),50,0,150,weight*fake);
                      plotFill("pt_distribution_of_SubLeading_#mu_with_"+totcharge[totchrg]+tauiso,muPt->at(vec_muele[jmu]),50,0,150,weight*fake);
                      plotFill("eta_distribution_of_Leading_#mu_with_"+totcharge[totchrg]+tauiso,muEta->at(vec_muele[imu]),20,-2.4,2.4,weight*fake);
                      plotFill("eta_distribution_of_SubLeading_#mu_with_"+totcharge[totchrg]+tauiso,muEta->at(vec_muele[jmu]),20,-2.4,2.4,weight*fake);
                      plotFill("pt_distribution_of_Leading_#mu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",muPt->at(vec_muele[imu]),50,0,150,weight);
                      plotFill("pt_distribution_of_SubLeading_#mu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",muPt->at(vec_muele[jmu]),50,0,150,weight);
                      plotFill("eta_distribution_of_Leading_#mu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",muEta->at(vec_muele[imu]),20,-2.4,2.4,weight);
                      plotFill("eta_distribution_of_SubLeading_#mu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",muEta->at(vec_muele[jmu]),20,-2.4,2.4,weight);
			...
			for(int j=0; j<2; j++) {
			  ...
			  plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso,H.M(),8,40,200,weight*fake*muonweight);
			  plotFill("InvariantMass_of_taumu_with_"+totcharge[totchrg]+tauiso+"_nofake_weight",H.M(),8,40,200,weight*muonweight);
			}
		      }
		    }
		    if(isys!=0){
		      plotFill("InvariantMass_of_4_particle_with_"+totcharge[totchrg]+tauiso+"_"+systematic,HH.M(),6,250,900,weight*leading_weight*subleading_weight*fake);
		      cout << isys << "\t" << t1.Pt() << "\t" << t2.Pt() << "\t" << HH.M() << endl;
		    }
		  }
		}
	      }
...
std::pair<double,double> tauPtE(double taupt, double tauenergy,int sys) {
  ...
  double pt = (sys==0 || sys==1) ? taupt : ((sys==2) ? taupt*tauESUp : taupt*tauESDn);
  double E = (sys==0 || sys==1) ?  tauenergy : ((sys==2) ? tauenergy*tauESUp : tauenergy*tauESDn);
