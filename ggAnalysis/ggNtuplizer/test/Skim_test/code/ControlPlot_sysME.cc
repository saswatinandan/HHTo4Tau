...
int  main(int argc, char** argv) {
  ...
  for(int k=1; k<input.size(); k++ ) {
	...
	bool mupt18(false);
	for  (int imu=0 ; imu < nMu; imu++) {
          bool mupt = (muPt->at(imu) >18) || (muPt->at(imu) >9);
	  mupt18 = (imu==0 && muPt->at(imu) >18);
          UShort_t id = (muIDbit->at(imu) >> 1 & 1);
          float IsoMu=muPFChIso->at(imu)/muPt->at(imu);
          if ( (muPFNeuIso->at(imu) + muPFPhoIso->at(imu) - 0.5* muPFPUIso->at(imu) )  > 0.0)
            IsoMu= ( muPFChIso->at(imu) + muPFNeuIso->at(imu) + muPFPhoIso->at(imu) - 0.5* muPFPUIso->at(imu))/muPt->at(imu);
	...
	for (int iele = 0; iele < nEle; ++iele) {
          ...
          bool elept = (mupt18) ? (elePt->at(iele) >13) : (elePt->at(iele) >18);
	...
	if(vec_tau.size() >1) {
	  ...
	  for(int itau=0; itau<vec_tau.size(); itau++) {
		...
		if((Muon || Electron) && pt[ipart] <=18) continue;
		...
		for(int jpart=jstart; jpart<vec_muele.size(); jpart++) {
		  ...
		  for(int partchrg=0; partchrg<2; partchrg++) {
			...
			plotFill("pt_distribution_of_Leading_lepton_with_"+title+tauiso,pt[ipart],30,0,150,weight*fake);
			plotFill("pt_distribution_of_SubLeading_lepton_with_"+title+tauiso,pt[jpart],30,0,150,weight*fake);
			plotFill("eta_distribution_of_Leading_lepton_with_"+title+tauiso,eta[ipart],20,-2.4,2.4,weight*fake);
			plotFill("eta_distribution_of_SubLeading_lepton_with_"+title+tauiso,eta[jpart],20,-2.4,2.4,weight*fake);
...
double fakeweight(double pt)  {
  ...
  // return .0191 +.230*TMath::Exp(-.0650*pt);///DYLmm
    return .1397*TMath::Exp(-.0360*pt);///DYLee
