...
GenParticleParentage(reco::GenParticleRef& _match) {
  //  cout << "GenParticleParentage" << _match << endl;
...
void GenParticleParentage::getParentageRecursive(const reco::GenParticleRef& p, int daughterId) {
  ...
  //bool write(false);
  //  if((p->pdgId()) ==-13) write = true;
  const int nmom = p->numberOfMothers();
  for( int i = 0; i < nmom; ++i ) {
    //    if(write)std::cout << "Entering" << std::endl;
    reco::GenParticleRef next = p->motherRef(i);
    //if(write)std::cout << "i= " << i << "pdgid= " << p->pdgId() << "status= " << p->status() << "pt= " << p->pt() << " momPid= " << next->pdgId() << "mompt= " << next->pt() << std::endl;
