...
ggNtuplizer::ggNtuplizer(const edm::ParameterSet& ps) {
  ...
  std::cout << " trgEventLabel_ " << ps.getParameter<InputTag>("triggerEvent") << " triggerObjectsLabel_ " << ps.getParameter<edm::InputTag>("triggerEvent") << " trgResultsLabel_ " << ps.getParameter<InputTag>("triggerResults") << " patTrgResultsLabel_ " << ps.getParameter<InputTag>("patTriggerResults") << std::endl;
  ...
  htrig = fs->make<TH1F>("trigger",    "",   4,  0.,   4.);
